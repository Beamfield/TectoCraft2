...
import com.beamfield.tc2.common.TC2Log;
...
import com.beamfield.tc2.common.tileentity.TileEntityTectonicOven;
import com.beamfield.tc2.common.tileentity.TileEntityTectonicSmasher;
import com.beamfield.tc2.utils.Reference;
import com.beamfield.tc2.utils.StaticUtils;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class BlockTC2Device extends BlockCoFHBase{
public static final int META_TECTONICOVEN = 0;
public static final int META_TECTONICSMASHER = 1;
	...
	public BlockTC2Device(Material material) {
		super(material);
		setStepSound(soundTypeMetal);
		setHardness(1.0f);
		setBlockName("blockTC2Device");
		setBlockTextureName(Reference.TEXTURE_NAME_PREFIX + "blockTC2Device");
	...
	private IIcon safeGetIcon(IIcon[] list, int idx, int x, int y, int z) {
		if(idx < 0 || idx >= list.length) {
			TC2Log.warning("Invalid metadata (%d) for block at %d, %d, %d!", idx, x, y, z);
			return blockIcon;
		}
		else {
			return list[idx];
	...
	public IIcon getIconFromTileEntity(TileEntity te, int metadata, int side) {
		...
		if(side == front) {
			if(te instanceof TileEntityTC2Base) {
				TileEntityTC2Base beefTe = (TileEntityTC2Base)te;
				if(beefTe.isActive()) {
		...
		if(te instanceof ITC2ReconfigurableSides) {
			ITC2ReconfigurableSides teSides = (ITC2ReconfigurableSides)te;
	...
	public TileEntity createNewTileEntity(World world, int metadata) {
		switch(metadata) {
		case META_TECTONICOVEN:
			return new TileEntityTectonicOven();
		case META_TECTONICSMASHER:
	...
	public ItemStack getTectonicOvenItemStack() {
		return new ItemStack(this, 1, META_TECTONICOVEN);
	}
	
	public ItemStack getTectonicSmasherItemStack() {
		return new ItemStack(this, 1, META_TECTONICSMASHER);
	}
	
	@Override
	public void getSubBlocks(Item par1, CreativeTabs par2CreativeTabs, List par3List)
	{
		par3List.add(this.getTectonicOvenItemStack());
		par3List.add(this.getTectonicSmasherItemStack());
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ) {
		...
		if(te instanceof TileEntityTC2Base) {
			if(!world.isRemote) {
				entityPlayer.openGui(TC2.modInstance, 0, world, x, y, z);
			}
			return true;
		}
