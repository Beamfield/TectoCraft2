...
protected int x, y, z;
	...
	public abstract static class Handler<M extends WorldMessage> implements IMessageHandler<M, IMessage> {
		...
		public IMessage onMessage(WorldMessage message, MessageContext ctx) {
			...
			if(world == null) {
				TC2Log.fatal("Unable to resolve world from messagecontext for WorldMessage");
				return null;
			}
			
			TileEntity te = world.getTileEntity(message.x, message.y, message.z);
			if(te == null) {
				TC2Log.error("Unable to find tile entity for WorldMessage at %d, %d, %d", message.x, message.y, message.z);
				return null;
			}
